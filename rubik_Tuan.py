...
def GeneticAlgo(rubik : Rubik):
    ...
    def crossover(p1, p2):
        ...
        return c1, c2
    def mutate(r):
        mu_rate = .3 # mutate about 20 percent of moves
        for i in range(num_moves_goal):
            idx = num_moves_goal - 1 - i # mutate from right to left
            if random.random() <= mu_rate:
                r[idx] = random.choice(moves)
            mu_rate *= .9 # decrease mutation rate by 10 percent for next move
        return r
    def moves_eval_fitness(route):
        # print(route, "".join(list(filter(lambda x: x != 'N', route))))
        new_rubik = rubik.copy()
        new_rubik.moves("".join(list(filter(lambda x: x != 'N', route))))
        return fitness_func(new_rubik)
    ...
    for _ in range(num_pop):
        route = ""
        route = [random.choice(moves) for _ in range(num_moves_goal)]
        chromosome = Chromosome(route)
        chromosome.set_fitness(moves_eval_fitness(route))
        pop.append(chromosome)

    a = 0
    while True:
        # Selection and crossover
        pop.sort(key=Chromosome.get_fitness)
        a+=1
        if a == 100:
            print([p.get_fitness() for p in pop])
            a = 0
        if (pop[0] == 0):
            return "".join(pop[0].route)
        pivot = num_pop - 1
        for i in range(0, (num_pop - num_mates)//2):
            c1, c2 = crossover(pop[i].get_route(), pop[(i + (num_pop - num_mates)//2) % num_pop].get_route())
            child1, child2 = Chromosome(c1), Chromosome(c2)
            child1.set_fitness(moves_eval_fitness(c1))
            child2.set_fitness(moves_eval_fitness(c2))
            if child1.get_fitness() < pop[pivot].get_fitness():
                pop[pivot] = child1
                pivot -= 1
            if child2.get_fitness() < pop[pivot].get_fitness():
                pop[pivot] = child2
                pivot -= 1
        ...
        for chromosome in pop:
            if random.random() <= mutation_rate:
                chromosome.set_route(mutate(chromosome.get_route()))
                chromosome.set_fitness(moves_eval_fitness(chromosome.get_route()))
